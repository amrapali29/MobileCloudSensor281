...
io.on('connection',function(socket){
...

    {
        ...
        pool.getConnection(function(err,connection){
            ...
            connection.query(" select hourlycharges, dailycharges, monthlycharges, yearlycharges from pricedetail where providerID=1    ",function(err,rows){
                connection.release();
                if(!err) {
                    console.log("abcdefghijklmnopqrstuvwxyz--------");
                    console.log(rows);
...
io.on('connection',function(socket){
    console.log("A user is connected price table.. html page..--");
    getpriceData(function (err, h) {
        socket.emit('showrows5', h);
        socket.on('savePrice',function(data)
