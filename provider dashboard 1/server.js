...
function getnotificationData(callback) {
    pool.getConnection(function(err,connection){
        ...
        var z = "Select consumerID , providerID,requestDate  ,  nodeCount, status  from requestdb ";
    ...
    console.log("A user is connected--Pricing Page:---");
        ...
        socket.on('savePrice',function(data){
            ...
            pool.getConnection(function(err,connection){
                ...
                connection.query(" select hourlycharges, dailycharges, monthlycharges, yearlycharges from pricedetail where providerID",tmpPriceData,function(err,rows){
                    connection.release();
                    if(!err) {
                        console.log("abcdefghijklmnopqrstuvwxyz--------");
                        console.log(rows);
...
// Consumer list html page table
 ...
 function getConsumerData(callback) {
     pool.getConnection(function (err, connection) {
         ...
         connection.query("Select fname,consumerID, emailID, Address, contactHome from db_consumer", function (err, rows) {
             console.log("bhkjhkhk");
             connection.release();
             if (!err) {
